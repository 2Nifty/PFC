...
{
    if(document.getElementById("hidShowWorkSheet").value=='true')
    {
        if(document.getElementById("txtPO").value !="" && document.getElementById("CustDet_txtCustNo").value !="" && document.getElementById("hidCommandLineReadOnly").value =="false")
        {
                pricehwind = window.open("PriceWorksheet.aspx",'WorkSheet' ,'height=560,width=1000,scrollbars=no,status=no,top='+((screen.height/2) - (590/2))+',left='+((screen.width/2) - (1020/2))+',resizable=yes,scrollbars=no','');
                pricehwind.focus();
                pricehwind.location.reload();
        }
    }
    else
        alert('Work Sheet not Available');
}
