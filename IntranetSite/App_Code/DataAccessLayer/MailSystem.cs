...
namespace PFC.Intranet.BusinessLogicLayer
{
    ...
    public class MailSystem : System.Web.UI.Page
    {
        ...
        protected string smtpServer = "mail.novantus.com";
        ...
        private void GetControlBlockData()
        {
            //
            // Select Mail server from DB
            //
            string _tableName = "SDK_Applicationconfiguration";
            string _columnNames = "SmtpServer,SiteURL";

            //
            // Get smtp server for the user
            //
            DataSet dsConfigDetails = SqlHelper.ExecuteDataset(Global.QuotesSystemConnectionString, SP_GENERALSELECT,
                                    new SqlParameter("@tableName", _tableName),
                                    new SqlParameter("@columnNames", _columnNames),
                                    new SqlParameter("@whereCondition", "1=1"));

            DataRow drConfigDetails = dsConfigDetails.Tables[0].Rows[0];

            smtpServer = drConfigDetails["SmtpServer"].ToString();

            siteURL = drConfigDetails["SiteURL"].ToString();
        ...
        private string GetFromAddress(string templateID)
        {
            ...
            DataSet dsFromAddress = SqlHelper.ExecuteDataset(Global.UmbrellaConnectionString, SP_GENERALSELECT,
                                     new SqlParameter("@tableName", _tableName),
                                     new SqlParameter("@columnNames", _columnNames),
        ...
        public void SendMail(string To, string CC, string BCC, string TemplateID, string Subject,string userName,string Name,string password)
        {
            try
            {
                ...
                //
                string _bodyMessage = GetContent(TemplateID);
                _bodyMessage = _bodyMessage.Replace("[Name]", Name);
                _bodyMessage = _bodyMessage.Replace("[Login Userid]", userName);
                _bodyMessage = _bodyMessage.Replace("[Password]", password);
        ...
        private string GetContent(string TemplateID)
        {
            ...
            try
            {
                ...
                if (dsEmailContent.Tables[0].Rows.Count > 0)
                {
                    ...
                    EmailContent = EmailContent.Replace("[SiteURL]",Global.IntranetSiteURL.ToString());
                }
                return EmailContent;
            }
            catch (Exception ex)
        ...
        private string GetSmtpServer()
        {
            try
            {
                ...
                string _tableName = "UCOR_ApplicationConfiguration";
                string _columnName = "SmtpServer";
                string _whereClause = "1=1";
                string smtpName = string.Empty;

                smtpName =(string)SqlHelper.ExecuteScalar(Global.UmbrellaConnectionString, "UGEN_SP_Select",
                                                new SqlParameter("@tableName", _tableName),
