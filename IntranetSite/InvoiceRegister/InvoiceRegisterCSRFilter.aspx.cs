...
public partial class InvoiceRegister : System.Web.UI.Page
{
    ...
    InvoiceRegisterReportTesting invoice = new InvoiceRegisterReportTesting();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Ajax.Utility.RegisterTypeForAjax(typeof(InvoiceRegister));
        ...
        if (!IsPostBack)
        {
            ...
            BindBranch();
            dtDate = invoice.GetCurrentDateValue();
            if (dtDate != null && dtDate.Rows.Count > 0)
            {
                cldEndDt.SelectedDate = Convert.ToDateTime(dtDate.Rows[0]["EndDate"].ToString());
                cldStartDt.SelectedDate = Convert.ToDateTime(dtDate.Rows[0]["BegDate"].ToString());
                cldEndDt.VisibleDate = Convert.ToDateTime(dtDate.Rows[0]["EndDate"].ToString());
                cldStartDt.VisibleDate = Convert.ToDateTime(dtDate.Rows[0]["BegDate"].ToString());
            }
    ...
    {
        invoice.BindListControls(ddlBranch, "Name", "Code", invoice.GetBranchLocation(), "ALL");
    }

    private void BindDataGrid()
    {
        dtMargin = invoice.GetMarginValue();
        ...
        if (sort == "")
        {
            lblStartDt.Text = cldStartDt.SelectedDate.ToShortDateString();
            _startDate = cldStartDt.SelectedDate.ToShortDateString();
            lblEndDt.Text = cldEndDt.SelectedDate.ToShortDateString();
            ...
            string _customerNo = (ddlCustNo.Items.Count >0 ? ddlCustNo.SelectedItem.Value: "");
            string _CSRName = (ddlCSR.Items.Count >0 ? ddlCSR.SelectedItem.Value: "");
            dtInvoice = invoice.GetInvoiceData(_startDate, _endDate, _customerNo, _CSRName);
        ...
        {
            dtInvoice.DefaultView.RowFilter = (ddlBranch.SelectedItem.Text.ToUpper() != "ALL") ? "BranchSls='" + ddlBranch.SelectedItem.Value + "'" : "";
    ...
    private void BindAdvanceFilters()
    {
        ListItem item = new ListItem("ALL", "");

        dtInvoice.DefaultView.RowFilter = (ddlBranch.SelectedItem.Text.ToUpper() != "ALL") ? "BranchSls='" + ddlBranch.SelectedItem.Value + "'" : "";

        dtCustNo = dtInvoice.DefaultView.ToTable(true, "SellToCustomerNumber");
        dtCSR = dtInvoice.DefaultView.ToTable(true, "CustomerSrvcRepName");
        dtCSR.DefaultView.Sort = "CustomerSrvcRepName";
        dtCustNo.DefaultView.Sort = "SellToCustomerNumber Asc";

        ddlCSR.DataSource = dtCSR.DefaultView.ToTable();
        ddlCSR.DataValueField = "CustomerSrvcRepName";
        ddlCSR.DataBind();
        ddlCSR.Items.Insert(0, item);

        ddlCustNo.DataSource = dtCustNo.DefaultView.ToTable();
        ddlCustNo.DataValueField = "SellToCustomerNumber";
        ddlCustNo.DataBind();
        ddlCustNo.Items.Insert(0, item);

        ddlCustNo.ClearSelection();
    ...
    {
        string _whereClause = "";
        if (ddlCSR.Items.Count > 0 && ddlCSR.SelectedItem.Text != "ALL")
            _whereClause += " and CustomerSrvcRepName='" + ddlCSR.SelectedItem.Value + "'";
        if (ddlCustNo.Items.Count > 0 && ddlCustNo.SelectedItem.Text != "ALL")
            _whereClause += " and SellToCustomerNumber='" + ddlCustNo.SelectedItem.Value + "'";
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (dtInvoiceBranch.Rows.Count > 0)
            {
                ...
                if (ddlSubTotals.SelectedItem.Text != "ALL")
                {
                    ...
                    foreach (DataRow dr in dtGroup.Rows)
                    {
                        try
                        {
                            ...
                            {
                                drSum["DisplayRplGMPct"] = (Math.Round((Math.Round(Convert.ToDecimal(dtFiltered.Compute("sum(NetUnitPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtFiltered.Compute("sum(RplCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtFiltered.Compute("sum(NetUnitPrice)", "")), 1)) * 100, 1));
                            }
                ...
                else
                {
                        ...
                        try
                        {
                            ...
                            foreach (DataRow drCustomer in dtCustomerGroup.Rows)
                            {
                                ...
                                foreach (DataRow drInvoice in dtInvoiceGroup.Rows)
                                {
                                    ...
                                    if (Convert.ToDecimal(dtInvoiceFiltered.Compute("sum(ExtendedPrice)", "")) == 0)
                                    {
                                        drSum["DisplayRplGMPct"] = 0;
                                    }
                                    else
                                    {
                                        drSum["DisplayRplGMPct"] = (Math.Round((Math.Round(Convert.ToDecimal(dtInvoiceFiltered.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtInvoiceFiltered.Compute("sum(RplCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtInvoiceFiltered.Compute("sum(ExtendedPrice)", "")), 1)) * 100, 1));
                                        //drSum["DisplayRplGMPct"] = (Math.Round((Math.Round(Convert.ToDecimal(dtInvoiceFiltered.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtInvoiceFiltered.Compute("sum(RplCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtInvoiceFiltered.Compute("sum(ExtendedPrice)", "")), 1)) * 100, 1));
                                    }
                                    dtMain.Rows.Add(drSum);
                                    //Response.Write( );
                                ...
                                if (Convert.ToDecimal(dtCustomerFiltered.Compute("sum(ExtendedPrice)", "")) == 0)
                                {
                                    drSum["DisplayRplGMPct"] = 0;
                                }
                                else
                                {
                                    drSum["DisplayRplGMPct"] = (Math.Round((Math.Round(Convert.ToDecimal(dtCustomerFiltered.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtCustomerFiltered.Compute("sum(RplCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtCustomerFiltered.Compute("sum(ExtendedPrice)", "")), 1)) * 100, 1));
                                }
                            ...
                            if (Convert.ToDecimal(dtRepFiltered.Compute("sum(ExtendedPrice)", "")) == 0)
                            {
                                drSum["DisplayRplGMPct"] = 0;
                            }
                            else
                            {
                                drSum["DisplayRplGMPct"] = (Math.Round((Math.Round(Convert.ToDecimal(dtRepFiltered.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtRepFiltered.Compute("sum(RplCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtRepFiltered.Compute("sum(ExtendedPrice)", "")), 1)) * 100, 1));
                            }
                            dtMain.Rows.Add(drSum);
                    ...
                    {
                        drSum["DisplayRplGMPct"] = (Math.Round((Math.Round(Convert.ToDecimal(dtInvoiceBranch.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtInvoiceBranch.Compute("sum(RplCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtInvoiceBranch.Compute("sum(ExtendedPrice)", "")), 1)) * 100, 1));
                    }
                    dtMain.Rows.Add(drSum);
    ...
    protected void gvInvoiceRegister_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            DataTable dtTotal = (ddlBranch.SelectedItem.Text == "ALL") ? dtInvoice.Copy() : dtInvoiceBranch.Copy();
            if (gvBranch.PageIndex == totalBranchFound)
            {
                ...
                if (Convert.ToDecimal(dtTotal.Compute("sum(ExtendedPrice)", "")) == 0)
                {
                    MarginRplPct = 0;
                }
                else
                {
                    MarginRplPct = (Math.Round((Math.Round(Convert.ToDecimal(dtTotal.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtTotal.Compute("sum(RplCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtTotal.Compute("sum(ExtendedPrice)", "")), 1)) * 100, 1));
                }
    ...
    protected void ibtnRunReport_Click(object sender, ImageClickEventArgs e)
    {
        ...
        else
        {
            ...
            DisplaStatusMessage("Invalid Date Range", "Fail");
            if (hidShowMode.Value == "Show")
    ...
    protected void ddlBranch_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            if (ddlCSR.Items.Count > 0)
                ddlCSR.SelectedIndex = 0;
            if (ddlCustNo.Items.Count > 0)
                ddlCustNo.SelectedIndex = 0;
        }

        BindDataGrid();
        
        // Fill advance filter tables
        BindAdvanceFilters();
    }
    ...
    protected void ibtnPrint_Click(object sender, ImageClickEventArgs e)
    {
        ...
        headerContent += "<tr><th colspan='18' style='color:blue' align=left><center>Invoice Register Report</center></th></tr>";
        ...
        headerContent += "<td style='padding-bottom:15px;'><b>Show Sub-Totals:" + ddlSubTotals.SelectedItem.Text + "</b></td><td style='padding-bottom:15px;'><b>Branch:" + ddlBranch.SelectedItem.Text + "</b></td>" +                
                         "<td  colspan='2' style='padding-bottom:15px;' ><b>CSR: " + ddlCSR.SelectedItem.Text + "</b></td>" +
                        "<td  colspan='2' style='padding-bottom:15px;' ><b>Customer No: " + ddlCustNo.SelectedItem.Text + "</b></td>" +
    ...
    private DataTable GetExcelTable(DataTable dtValue)
    {
        ...
        {
            dtValue.DefaultView.RowFilter = (ddlBranch.SelectedItem.Text.ToUpper() != "ALL") ? "BranchSls='" + ddlBranch.SelectedItem.Value + "'" : "";
                ...
                foreach (DataRow drBranch in dtBranchGroup.Rows)
                {
                    ...
                    if (dtInvoiceBranch.Rows.Count > 0)
                    {
                        if (ddlSubTotals.SelectedItem.Text != "ALL")
                        {
                            ...
                            foreach (DataRow dr in dtGroup.Rows)
                            {
                                try
                                {
                                    ...
                                    {
                                        drSum["DisplayRplGMPct"] = Math.Round((Math.Round(Convert.ToDecimal(dtFiltered.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtFiltered.Compute("sum(RplCostDol)", "")), 1)) / (Convert.ToDecimal(dtFiltered.Compute("sum(ExtendedPrice)", ""))) * 100, 1);
                                    }
                                    if (ddlSubTotals.SelectedValue == "SellToCustomerNumber")
                                        drSum["SellToCustomerName"] = dtFiltered.Rows[0]["SellToCustomerName"];
                        ...
                        else
                        {
                                ...
                                try
                                {
                                    ...
                                    foreach (DataRow drCustomer in dtCustomerGroup.Rows)
                                    {
                                        ...
                                        foreach (DataRow drInvoice in dtInvoiceGroup.Rows)
                                        {
                                            ...
                                            {
                                                drSum["DisplayRplGMPct"] = Math.Round((Math.Round(Convert.ToDecimal(dtInvoiceFiltered.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtInvoiceFiltered.Compute("sum(RplCostDol)", "")), 1)) / (Convert.ToDecimal(dtInvoiceFiltered.Compute("sum(ExtendedPrice)", ""))) * 100, 1);
                                            }
                                    ...
                                    {
                                        drSum["DisplayRplGMPct"] = Math.Round((Math.Round(Convert.ToDecimal(dtRepFiltered.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtRepFiltered.Compute("sum(RplCostDol)", "")), 1)) / (Convert.ToDecimal(dtRepFiltered.Compute("sum(ExtendedPrice)", ""))) * 100, 1);
                                    }
                            ...
                            {
                                drSum["DisplayRplGMPct"] = (Convert.ToDecimal(dtInvoiceBranch.Compute("sum(ExtendedPrice)", "")) == 0 ? 0 : (Math.Round((Math.Round(Convert.ToDecimal(dtInvoiceBranch.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtInvoiceBranch.Compute("sum(RplCostDol)", "")), 1)) / (Convert.ToDecimal(dtInvoiceBranch.Compute("sum(ExtendedPrice)", ""))) * 100, 1)));
                            }
    ...
    protected void ibtnExcelExport_Click(object sender, ImageClickEventArgs e)
    {
        ...
        headerContent += "<tr><th colspan='19' style='color:blue' align=left><center>Invoice Register Report</center></th></tr>";
        headerContent += "<tr><td colspan='2'><b>StartDate :" + _startDate + "</b></td><td  colspan='2'><b>End Date:" + _endDate + "</b></td>";
        headerContent += "</b></td><td  colspan='2'><b>Show Sub-Totals:" + ddlSubTotals.SelectedItem.Text + "</b></td><td><b>Branch:" + ddlBranch.SelectedItem.Text + "</b></td>" +
                       "<td  colspan='2' style='padding-bottom:15px;' ><b>CSR: " + ddlCSR.SelectedItem.Text + "</b></td>" +
                        "<td  colspan='2' style='padding-bottom:15px;' ><b>Customer No: " + ddlCustNo.SelectedItem.Text + "</b></td>" +  
    ...
    protected void dv_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.Footer)
        {
            DataTable dt = (ddlBranch.SelectedItem.Text == "ALL") ? (DataTable)Session["InvoiceRegisterExcel"] : dtInvoiceBranch;
            ...
            {
                MarginRplPct = (Math.Round((Math.Round(Convert.ToDecimal(dt.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dt.Compute("sum(RplCostDol)", "")), 1)) / (Convert.ToDecimal(dt.Compute("sum(ExtendedPrice)", ""))) * 100, 1));
            }
            e.Row.Cells[13].Text = string.Format("{0:#,##0.0}", MarginRplPct);
    ...
    protected void ddlCustNo_SelectedIndexChanged(object sender, EventArgs e)
    {
        BindDataGrid();
    }
}
