...
public partial class InvoiceRegister : System.Web.UI.Page
{
                ...
                if (ddlSubTotals.SelectedItem.Text != "ALL")
                {
                    ...
                    foreach (DataRow dr in dtGroup.Rows)
                    {
                        try
                        {
                            ...
                            {
                                drSum["DisplayRplGMPct"] = (Math.Round((Math.Round(Convert.ToDecimal(dtFiltered.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtFiltered.Compute("sum(RplCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtFiltered.Compute("sum(ExtendedPrice)", "")), 1)) * 100, 1));
                            }
    ...
    protected void gvInvoiceRegister_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            DataTable dtTotal = (DataTable)Session["InvoiceRegisterExcel"];
                            ...
                            foreach (DataRow dr in dtRepGroup.Rows)
                            {
                                try
                                {
                                    ...
                                    foreach (DataRow drCustomer in dtCustomerGroup.Rows)
                                    {
                                        ...
                                        {
                                            drSum["DisplayRplGMPct"] = Math.Round((Math.Round(Convert.ToDecimal(dtCustomerFiltered.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtCustomerFiltered.Compute("sum(RplCostDol)", "")), 1)) / (Convert.ToDecimal(dtCustomerFiltered.Compute("sum(ExtendedPrice)", "")) * 100), 1);
                                        }
                                        drSum["SellPerLb"] = ((Convert.ToDecimal(dtCustomerFiltered.Compute("sum(ExtendedWeight)", "")) == 0) ? 0 : Convert.ToDecimal(dtCustomerFiltered.Compute("sum(ExtendedPrice)", "")) / Convert.ToDecimal(dtCustomerFiltered.Compute("sum(ExtendedWeight)", "")));
                                        drSum["SellToCustomerNumber"] = dtCustomerFiltered.Rows[0]["SellToCustomerNumber"];
                                        drSum["SellToCustomerName"] = dtCustomerFiltered.Rows[0]["SellToCustomerName"];
    ...
    {
        PrintCtl.PageTitle = "Invoice Register for " + Session["UserName"].ToString();

        string pageURL = "InvoiceRegister/InvoiceRegisterFilterTableExport.aspx?UserName=" + Session["UserName"].ToString();
        ...
        pageURL += "&FilterType=" + FilterDropDownList.SelectedValue;
        pageURL += "&FilterTypeText=" + FilterDropDownList.SelectedItem.Text;

        PrintCtl.EnableFax = false;
        PrintCtl.PageUrl = pageURL;
        PrintCtl.FormName = "InvoiceRegisterFilterTable";
        pnlPrint.Update();
    }
