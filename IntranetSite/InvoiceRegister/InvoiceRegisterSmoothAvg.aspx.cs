...
public partial class InvoiceRegisterSmoothAvg : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Ajax.Utility.RegisterTypeForAjax(typeof(InvoiceRegisterSmoothAvg));
    ...
    private void BindDataGrid()
    {
        ...
        {
            //minMargin = ((Convert.ToInt32(dtMargin.Rows[0]["AppOptionValue"].ToString()))<(Convert.ToInt32(dtMargin.Rows[1]["AppOptionValue"].ToString())))?(Convert.ToInt32(dtMargin.Rows[0]["AppOptionValue"].ToString())):(Convert.ToInt32(dtMargin.Rows[1]["AppOptionValue"].ToString()));
            //maxMargin = ((Convert.ToInt32(dtMargin.Rows[0]["AppOptionValue"].ToString())) > (Convert.ToInt32(dtMargin.Rows[1]["AppOptionValue"].ToString()))) ? (Convert.ToInt32(dtMargin.Rows[0]["AppOptionValue"].ToString())) : (Convert.ToInt32(dtMargin.Rows[1]["AppOptionValue"].ToString()));
        ...
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (ddlSubTotals.SelectedItem.Text != "ALL")
                {
                    ...
                    foreach (DataRow dr in dtGroup.Rows)
                    {
                        try
                        {
                            ...
                            if (Convert.ToDecimal(dtFiltered.Compute("sum(ExtendedPrice)", "")) == 0)
                            {
                                drSum["StdCostPct"] = 0;
                            }
                            else
                            {
                                drSum["StdCostPct"] = (Math.Round((Math.Round(Convert.ToDecimal(dtFiltered.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtFiltered.Compute("sum(StdCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtFiltered.Compute("sum(ExtendedPrice)", "")), 1)) * 100, 1));
                            }
                ...
                else
                {
                        ...
                        try
                        {
                            ...
                            foreach (DataRow drCustomer in dtCustomerGroup.Rows)
                            {
                                ...
                                foreach (DataRow drInvoice in dtInvoiceGroup.Rows)
                                {
                                    ...
                                    {
                                        drSum["StdCostPct"] = (Math.Round((Math.Round(Convert.ToDecimal(dtInvoiceFiltered.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtInvoiceFiltered.Compute("sum(StdCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtInvoiceFiltered.Compute("sum(ExtendedPrice)", "")), 1)) * 100, 1));
                                    }                                    
                                    dtMain.Rows.Add(drSum);
                                    //Response.Write( );
                                }
                                ...
                                {
                                    drSum["StdCostPct"] = (Math.Round((Math.Round(Convert.ToDecimal(dtCustomerFiltered.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtCustomerFiltered.Compute("sum(StdCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtCustomerFiltered.Compute("sum(ExtendedPrice)", "")), 1)) * 100, 1));
                                }
                            ...
                            {
                                drSum["StdCostPct"] = (Math.Round((Math.Round(Convert.ToDecimal(dtRepFiltered.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtRepFiltered.Compute("sum(StdCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtRepFiltered.Compute("sum(ExtendedPrice)", "")), 1)) * 100, 1));
                            }
                            dtMain.Rows.Add(drSum);
                    ...
                    {
                        drSum["StdCostPct"] = (Math.Round((Math.Round(Convert.ToDecimal(dtInvoiceBranch.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtInvoiceBranch.Compute("sum(StdCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtInvoiceBranch.Compute("sum(ExtendedPrice)", "")), 1)) * 100, 1));
                    }
                    dtMain.Rows.Add(drSum);
    ...
    protected void gvInvoiceRegister_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.Footer)
        {
            ...
            if (gvBranch.PageIndex == totalBranchFound)
            {
                ...
                {
                    MarginRplPct = (Math.Round((Math.Round(Convert.ToDecimal(dtTotal.Compute("sum(ExtendedPrice)", "")), 1) - Math.Round(Convert.ToDecimal(dtTotal.Compute("sum(StdCostDol)", "")), 1)) / (Math.Round(Convert.ToDecimal(dtTotal.Compute("sum(ExtendedPrice)", "")), 1)) * 100, 1));
                }
    ...
    protected void ibtnPrint_Click(object sender, ImageClickEventArgs e)
    {
        ...
        headerContent += "</b></td><td style='padding-bottom:15px;'><b>Show Sub-Totals:" + ddlSubTotals.SelectedItem.Text + "</b></td><td style='padding-bottom:15px;'><b>Branch:" + ddlBranch.SelectedItem.Text +
            "</b></td><td colspan='4' style='padding-bottom:15px;' ><b>Run By: " + Session["UserName"].ToString() + "&nbsp;&nbsp;  Run Date: " + DateTime.Now.ToShortDateString() + "</></td></tr>";
    ...
    protected void ibtnExcelExport_Click(object sender, ImageClickEventArgs e)
    {
        ...
        headerContent += "<tr><th colspan='19' style='color:blue' align=left><center>Smooth Average Invoice Register</center></th></tr>";
        headerContent += "<tr><td colspan='2'><b>StartDate :" + _startDate + "</b></td><td  colspan='2'><b>End Date:" + _endDate + "</b></td>";
        headerContent += "</b></td><td  colspan='2'><b>Show Sub-Totals:" + ddlSubTotals.SelectedItem.Text + "</b></td><td><b>Branch:" + ddlBranch.SelectedItem.Text +
            "</b></td><td colspan='2'><b>Run By: " + Session["UserName"].ToString() + "&nbsp;&nbsp;  Run Date: " + DateTime.Now.ToShortDateString() + "</></td></tr>";
        ...
        headerContent += "<tr><th nowrap width='70px'> <center>PostDate</center> </th>" +
                            ...
                            "<th width='40px'  nowrap align='center' nowrap  ><table border='1px' ><tr><th colspan='2'>Margin %</th></tr><tr><th>Avg</th><th>Smooth Avg</th></table></th>" +
    ...
    {
        PrintCtl.PageTitle = "Smooth Avg Invoice Register for " + Session["UserName"].ToString();

        string pageURL = "InvoiceRegister/InvoiceRegisterSmoothAvgExport.aspx?UserName=" + Session["UserName"].ToString();
        pageURL += "&StartDate=" + _startDate;
        pageURL += "&EndDate=" + _endDate;
        pageURL += "&SubTotalType=" + ddlSubTotals.SelectedValue;
        pageURL += "&SubTotalTypeDesc=" + ddlSubTotals.SelectedItem.Text;
        pageURL += "&BranchID=" + ddlBranch.SelectedValue;
        pageURL += "&BranchDesc=" + ddlBranch.SelectedItem.Text;

        PrintCtl.EnableFax = false;
        PrintCtl.PageUrl = pageURL;
        PrintCtl.FormName = "SmoothAvgInvoiceRegister";
        pnlPrint.Update();
    }
