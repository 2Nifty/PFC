...
public partial class POPastDueReport : System.Web.UI.Page
{
    ...
    {
        Ajax.Utility.RegisterTypeForAjax(typeof(POPastDueReport));
        Response.Cache.SetCacheability(HttpCacheability.NoCache);
        reportGrp = Request.QueryString["ReportGrp"].ToString();

        if (!IsPostBack)
        {
            lblReportGroup.Text = reportGrp;
            hidFileName.Value = "POPastDueReport" + Session["SessionID"].ToString() + ".xls";
            BindDataGrid();
        }
    }
    ...
    {
        dtPOData = poPastDue.GetPOPastDueData("popostdue", reportGrp);
    ...
    protected void gvPOPastDue_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.Header)
        {
            ...
            e.Row.Cells[2].Text = "<table border='0' cellpadding='0' cellspacing='0'  width='100%'><tr><td class='GridHead splitBorder' style='border-bottom:solid 1px #c9c6c6;border-right:solid 0px #c9c6c6;' colspan=3 nowrap ><center>Total Open PO</center></td></tr><tr>"+
                                        ...
                                        "<Div style='cursor:hand;' onclick=\"javascript:BindValue('TotOpenPOQty');\">Qty</div></center></td><td width='75' class='GridHead splitBorders' nowrap align='center' nowrap  style='cursor:hand;border-right:solid 1px #c9c6c6;'>" +
                                        "<Div style='cursor:hand;' onclick=\"javascript:BindValue('TotOpenPOLbs');\">Pounds</div></center></td><td width='55' class='GridHead splitBorders' nowrap align='center' nowrap  style='cursor:hand;border-right:solid 0px #c9c6c6;'>" +
        ...
        {
            HyperLink hplCatGroup = e.Row.FindControl("hplReportGrp") as HyperLink;
            hplCatGroup.ForeColor = System.Drawing.Color.Blue;
            hplCatGroup.Attributes.Add("onclick", "var hWnd=window.open(this.href, 'popupwindow2', 'height=710,width=1020,scrollbars=no,status=no,top='+((screen.height/2) - (750/2))+',left='+((screen.width/2) - (1020/2))+',resizable=no'); hWnd.opener = self; if (window.focus) {hWnd.focus()} return false;");
        }
        if(e.Row.RowType == DataControlRowType.Footer)
        {
            ...
            e.Row.Cells[2].Text = String.Format("{0:#,##0}", Convert.ToDecimal(dtGrandTotal.Compute("sum(TotOpenPOQty)", "").ToString()));
            e.Row.Cells[3].Text = String.Format("{0:#,##0.00}", Convert.ToDecimal(dtGrandTotal.Compute("sum(TotOpenPOLbs)", "").ToString()));
            e.Row.Cells[4].Text = String.Format("{0:#,##0}", Convert.ToDecimal(dtGrandTotal.Compute("sum(OpenPOMosSupply)", "").ToString()));
            e.Row.Cells[5].Text = String.Format("{0:#,##0.00}", Convert.ToDecimal(dtGrandTotal.Compute("sum(PastDueLbs)", "").ToString()));
            e.Row.Cells[6].Text = String.Format("{0:#,##0}", Convert.ToDecimal(dtGrandTotal.Compute("sum(PastDueMos)", "").ToString()));

            e.Row.Cells[7].Text = String.Format("{0:#,##0.00}", Convert.ToDecimal(dtGrandTotal.Compute("sum(CurLbs)", "").ToString()));
            e.Row.Cells[8].Text = String.Format("{0:#,##0}", Convert.ToDecimal(dtGrandTotal.Compute("sum(CurMos)", "").ToString()));
            e.Row.Cells[9].Text = String.Format("{0:#,##0.00}", Convert.ToDecimal(dtGrandTotal.Compute("sum(Fut1Lbs)", "").ToString()));
            e.Row.Cells[10].Text = String.Format("{0:#,##0}", Convert.ToDecimal(dtGrandTotal.Compute("sum(Fut1Mos)", "").ToString()));
            e.Row.Cells[11].Text = String.Format("{0:#,##0.00}", Convert.ToDecimal(dtGrandTotal.Compute("sum(Fut2Lbs)", "").ToString()));
            e.Row.Cells[12].Text = String.Format("{0:#,##0}", Convert.ToDecimal(dtGrandTotal.Compute("sum(Fut2Mos)", "").ToString()));
            e.Row.Cells[13].Text = String.Format("{0:#,##0.00}", Convert.ToDecimal(dtGrandTotal.Compute("sum(Fut3Lbs)", "").ToString()));
            e.Row.Cells[14].Text = String.Format("{0:#,##0}", Convert.ToDecimal(dtGrandTotal.Compute("sum(Fut3Mos)", "").ToString()));
    ...
    private string GenerateExportData(string dataFormat)
    {
        ...
        headerContent += "<tr><th colspan='15' style='color:blue' align=left><center>PO Past Due By Buy Group</center></th></tr>";
        headerContent += "<tr><td colspan='5'><b>Report Group:" + reportGrp +"</b></td><td colspan='10' align=right><b>Run By: " + Session["UserName"].ToString() + "&nbsp;&nbsp;  Run Date: " + DateTime.Now.ToShortDateString() + "</></td></tr>";
        ...
        if (dtExcelData.Rows.Count > 0)
        {
            ...
            bfExcel.HeaderText = "Buy Grp";
            bfExcel.DataField = "BuyGrp";            
