...
public partial class POPastDueByReportGrp : System.Web.UI.Page
{
    ...
    {
        Ajax.Utility.RegisterTypeForAjax(typeof(POPastDueByReportGrp));
        Response.Cache.SetCacheability(HttpCacheability.NoCache);
       
        if (!IsPostBack)
        {
            hidFileName.Value = "POPastDueByReportGrp" + Session["SessionID"].ToString() + ".xls";
            BindDataGrid();
        }
    }
    ...
    {
        dtPOData = poPastDue.GetPOPastDueData("popostduebyreportgrp", "");
        ...
        if (dtPOData != null && dtPOData.Rows.Count > 0)
        {
            ...
            Session["PODueByRptGrpData"] = dtPOData.DefaultView.ToTable();
        }
        else
        {
    ...
    protected void gvPOPastDue_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.Header)
        {
            ...
            e.Row.Cells[1].Text = "<table border='0' cellpadding='0' cellspacing='0'  width='100%'><tr><td class='GridHead splitBorder' style='border-bottom:solid 1px #c9c6c6;border-right:solid 0px #c9c6c6;' colspan=3 nowrap ><center>Total Open PO</center></td></tr><tr>"+
                                        ...
                                        "<Div style='cursor:hand;' onclick=\"javascript:BindValue('TotOpenPOQty');\">Qty</div></center></td><td width='94' class='GridHead splitBorders' nowrap align='center' nowrap  style='cursor:hand;border-right:solid 1px #c9c6c6;'>" +
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            hplCatGroup.Attributes.Add("onclick", "var hWnd=window.open(this.href, 'popastduereport', 'height=710,width=1020,scrollbars=no,status=no,top='+((screen.height/2) - (750/2))+',left='+((screen.width/2) - (1020/2))+',resizable=no'); hWnd.opener = self; if (window.focus) {hWnd.focus()} return false;");
        }
        ...
        if(e.Row.RowType == DataControlRowType.Footer)
        {
            ...
            e.Row.Cells[9].Text = String.Format("{0:#,##0}", Convert.ToDecimal(dtGrandTotal.Compute("sum(Fut1Mos)", "").ToString()));
            e.Row.Cells[10].Text = String.Format("{0:#,##0.00}", Convert.ToDecimal(dtGrandTotal.Compute("sum(Fut2Lbs)","").ToString()));
            e.Row.Cells[11].Text = String.Format("{0:#,##0}", Convert.ToDecimal(dtGrandTotal.Compute("sum(Fut2Mos)", "").ToString()));
            e.Row.Cells[12].Text = String.Format("{0:#,##0.00}", Convert.ToDecimal(dtGrandTotal.Compute("sum(Fut3Lbs)","").ToString()));
            e.Row.Cells[13].Text = String.Format("{0:#,##0}", Convert.ToDecimal(dtGrandTotal.Compute("sum(Fut3Mos)", "").ToString()));            
    ...
    private string GenerateExportData(string dataFormat)
    {
        ...
        dtExcelData = Session["PODueByRptGrpData"] as DataTable;        
        ...
        headerContent += "<tr><th colspan='14' style='color:blue' align=left><center>PO Past Due By Report Group</center></th></tr>";
        headerContent += "<tr><td colspan='14' align=right><b>Run By: " + Session["UserName"].ToString() + "&nbsp;&nbsp;  Run Date: " + DateTime.Now.ToShortDateString() + "</></td></tr>";
        ...
        if (dtExcelData.Rows.Count > 0)
        {
            ...
            bfExcel.HeaderText = "Report Grp";
            bfExcel.DataField = "ReportGrp";            
