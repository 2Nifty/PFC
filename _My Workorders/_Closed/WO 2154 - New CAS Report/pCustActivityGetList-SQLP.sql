		...
		BEGIN
			...
			IF (@FilterCd = 'ChainCd')
				BEGIN
					...
					SELECT	DISTINCT
							isnull(Cust.ChainCd, Activity.ActivityChain) as RecID,
					...
					FROM	(SELECT	CA.LookupValue as ActivityChain,
							 ...
							 WHERE	CA.Period = @Period AND
									...
									isnull(CA.LookupValue,'') LIKE CASE isnull(@ChainCd,'') WHEN '' THEN '%' ELSE @ChainCd END) Activity INNER JOIN
							CustomerMaster Cust (NoLock)
					ON		Cust.ChainCd = Activity.ActivityChain LEFT OUTER JOIN
					...
					WHERE	isnull(Cust.CustShipLocation,'') LIKE CASE isnull(@CustShipLocation,'') WHEN '' THEN '%' ELSE @CustShipLocation END AND
							...
							isnull(RegionLoc.RepName,'') LIKE CASE isnull(@RegionalMgr,'') WHEN '' THEN '%' ELSE @RegionalMgr END AND
							isnull(Cust.BuyGroup,'') LIKE CASE isnull(@BuyGroup,'') WHEN '' THEN '%' ELSE @BuyGroup END
					ORDER BY isnull(Cust.ChainCd, Activity.ActivityChain)
				END
			...
			IF (@FilterCd = 'ChainXLS')
				BEGIN
					...
					FROM	(SELECT	CA.LookupValue as ActivityChain,
							 ...
							 WHERE	CA.Period = @Period AND
									CA.RecordType = 'Chain' AND
									isnull(CA.LookupValue,'') IN (SELECT Value FROM #ChainXLS)) Activity INNER JOIN
							CustomerMaster Cust (NoLock)
					ON		Cust.ChainCd = Activity.ActivityChain LEFT OUTER JOIN
							#tChainList Chain (NoLock)
					ON		Cust.ChainCd = Chain.ChainCd LEFT OUTER JOIN
...
--					SELECT Value FROM fListToTablevarchar(@ChainList,',') 
					...
					FROM	(SELECT	CA.LookupValue as ActivityChain,
							 ...
							 WHERE	CA.Period = @Period AND
									CA.RecordType = 'Chain' AND
									isnull(CA.LookupValue,'') IN (SELECT Value FROM fListToTablevarchar(@ChainList,','))) Activity INNER JOIN
							CustomerMaster Cust (NoLock)
					ON		Cust.ChainCd = Activity.ActivityChain LEFT OUTER JOIN
							#tChainList Chain (NoLock)
					ON		Cust.ChainCd = Chain.ChainCd LEFT OUTER JOIN
