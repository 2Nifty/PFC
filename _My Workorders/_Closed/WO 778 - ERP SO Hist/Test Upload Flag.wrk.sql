...
select
	[No_]				as	InvoiceNo
FROM	[Porteous$Sales Invoice Header] NVHDR
...
WHERE	NOT EXISTS
	(SELECT * FROM OpenDataSource('SQLOLEDB','Data Source=PFCSQLt;User ID=pfcnormal;Password=pfcnormal').PFCReports.dbo.SOHeaderHist
	 WHERE InvoiceNo=NVHDR.[No_] COLLATE SQL_Latin1_General_CP1_CI_AS)
AND	[Posting Date] > Cast('2006-08-26 00:00:00.000' as DATETIME)
...
FROM	OpenDataSource('SQLOLEDB','Data Source=PFCSQLt;User ID=pfcnormal;Password=pfcnormal').PFCTnT.dbo.[Porteous$Sales Invoice Line] NVLINE INNER JOIN
	SOHeaderHist ON NVLINE.[Document No_] = SOHeaderHist.InvoiceNo
WHERE	NOT EXISTS
	(SELECT * FROM SODetailHist INNER JOIN
			SOHeaderHist ON SODetailHist.fSOHeaderHistID = SOHeaderHist.pSOHeaderHistID
	 WHERE SOHeaderHist.InvoiceNo=NVLINE.[Document No_] COLLATE SQL_Latin1_General_CP1_CI_AS)
AND	[Posting Date] > Cast('2006-08-26 00:00:00.000' as DATETIME) AND Type=2 AND No_ <> '' 
...
WHERE	NOT EXISTS
	(SELECT * FROM OpenDataSource('SQLOLEDB','Data Source=PFCSQLt;User ID=pfcnormal;Password=pfcnormal').PFCReports.dbo.SOHeaderHist
	 WHERE InvoiceNo=NVHDR.[No_] COLLATE SQL_Latin1_General_CP1_CI_AS)
AND	NVHDR.[Posting Date] > Cast('2006-08-26 00:00:00.000' as DATETIME)
