...
public class InvoiceAnalysis
{
    ...
    public DataTable GetInvoiceAnalysisByCustNo(string startDate, string endDate, string orderType, string branchID, string chain, string customerNumber, string territory, string CSRName, string state, string shipment, string salesPerson, string priceCd, string orderSource, string allCustFlag)
    {
        try
        {
            ...
            //if (customerNumber != "")
            //    _whereCust += " AND tAllCust.CustNo='" + customerNumber + "'";
            if (territory != "")
                ...
                _whereCust += " AND tAllCust.CustNo in (SELECT CustNo FROM OpenDataSource('SQLOLEDB','Data Source=PFCERPDB;User ID=pfcnormal;Password=pfcnormal').PERP.dbo.CustomerMaster WHERE SalesTerritory='" + territory + "')";
            if (CSRName != "")
