...
--Reset ShipLoc, ShipLocName and IMLoc to null  (1 minutes)
UPDATE SOHeaderHist SET ShipLoc = null, ShipLocName = null WHERE SubType is null
UPDATE SODetailHist SET IMLoc = null FROM SOHeaderHist WHERE pSOHeaderHistID = fSOHeaderHistID and SubType is null


--UPDATE Empty SOHeaderHist.CustShipLoc from CustomerMaster  (1 minutes)
UPDATE	SOHeaderHist
...
WHERE	SubType is null AND Cust.CustNo = SellToCustNo AND
	(CustShipLoc = 0 OR CustShipLoc = '' or CustShipLoc is null)


--UPDATE SODetailHist.IMLoc FROM [Porteous$Sales Invoice Line].[Location Code]  (3 minutes)
UPDATE	SODetailHist
...
WHERE	pSOHeaderHistID = fSOHeaderHistID and SubType is null and LineNumber = NVLINE.Line


--UPDATE SODetailHist.IMLoc FROM [Porteous$Sales Cr_Memo Line].[Location Code]  (1 minutes)
UPDATE	SODetailHist
SET	IMLoc = NVLINE.Loc
FROM	SOHeaderHist INNER JOIN
	(SELECT	Lin.[Document No_] AS Doc, Lin.[Line No_] AS Line, Lin.[Location Code] AS Loc
...
--UPDATE SOHeaderHist.ShipLoc FROM SODetailHist.IMLoc  (1 minutes)
UPDATE	SOHeaderHist
SET	ShipLoc = Dtl.IMLoc
FROM	(SELECT	DISTINCT fSOHeaderHistID, IMLoc
	 FROM	SODetailHist
	 WHERE	IMLoc <> 0 and IMLoc <> '' and IMLoc is not null) Dtl
WHERE	pSOHeaderHistID = Dtl.fSOHeaderHistID and SubType is null
...
FROM	SOHeaderHist
WHERE	pSOHeaderHistID = fSOHeaderHistID and SubType is null and 
...
WHERE	Loc.LocID = ShipLoc and SubType is null
