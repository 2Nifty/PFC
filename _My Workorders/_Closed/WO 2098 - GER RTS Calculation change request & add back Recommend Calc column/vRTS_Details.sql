...
-- 	,case
--		when CPRDailyRTS.Use_30Day_Qty >0 then ISNULL(Avail_Qty/CPRDailyRTS.Use_30Day_Qty, 0)
--		else 0 end AS Avail_Mos
FROM	CPRDailyRTS with (NOLOCK) INNER JOIN
	GERRTSHdr with (NOLOCK) 
ON	CPRDailyRTS.LocationCode = GERRTSHdr.LocCd AND CPRDailyRTS.ItemNo = GERRTSHdr.ItemNo INNER JOIN
	LocMaster with (NOLOCK) 
