...
--(8725 row(s) affected)
--#tIBUList - All qualifying 020 usage to be moved
SELECT	tSKU.*
INTO	#tIBUList
...
-- ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ --
-- DEVPERP: Under 30 seconds to build arrive at the final Usage list to be moved (#tIBUList) --
-----------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------
...
--(69665 row(s) affected)
--#tOldUsage - Save Old ItemBranchUage transactions
SELECT	DISTINCT
...
--(69665 row(s) affected)
--Delete Old ItemBranchUage transactions that were saved
DELETE
--select *
...
--UPDATE usage for all existing ItemBranchUsage records
...
FROM	ItemBranchUsage IBU INNER JOIN
		(SELECT	CurPeriodNo,
				...
				--OldSellStkQty,
				--NewSellStkQty,
				--SUM(CurSalesQty) as TotQtyRaw,
				--SUM((CurSalesQty / OldSellStkQty) * NewSellStkQty) as TotCnvQty,
				--ROUND(SUM((CurSalesQty / OldSellStkQty) * NewSellStkQty),0) as TotRndQty,
