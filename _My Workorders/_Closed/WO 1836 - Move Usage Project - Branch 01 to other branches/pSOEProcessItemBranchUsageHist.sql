...
SELECT	ISNULL(SODH.UsageLoc,IMLoc) AS IMLoc,
	...
	SUM(NetWght * QtyShipped) AS SlsWght, @userName AS EntID, GetDate() AS EntDt
INTO	#SODetailSum
FROM	SODETAILHist SODH (nolock),
	SOHeaderHist SOHH (nolock) 
WHERE 	SODH.fSOHeaderHistID = SOHH.pSOHeaderHistID AND SOHH.InvoiceDt Between @begInvDt AND @endInvDt
GROUP BY ISNULL(SODH.UsageLoc,IMLoc), ItemNo
...
SELECT		SDS.IMLoc, SDS.ItemNo, SDS.Period,
		...
		Qty AS EndOH, ExtAvgCost AS EndVal, (Qty * [NetUnitWght]) As EndOHWght,
		0 AS NoOrd, 0 AS NRUse, 0 AS NRNoSls, 0 AS NRSlsDol, 0 AS NRSlsWght, 0 AS NRSlsCost,
		@userName EntID, GetDate() EntDt, NULL AS ChgID, NULL AS ChgDt, Null as StsCd 
FROM		#SODetailSum SDS (NOLOCK) LEFT OUTER JOIN
		SumItem SI (NOLOCK)
ON		SDS.IMLoc = SI.Location AND SDS.ItemNo = SI.ItemNo,
		ItemMaster IMM (NOLOCK)
WHERE		SDS.Period = @CurPeriod AND SI.SourceCd ='OH' AND SDS.ItemNo = IMM.ItemNo
...
--[CurBegOnHandQty] = BegQOH, [CurBegOnHandDol] = (BegQoh * BegAC),
--[CurBegOnHandWght] = BegQOH * [Wght]
--FROM PFCAC.dbo.AvgCst_DailyHist ACD (NOLOCK), ItemMaster IM (NOLOCK)
--WHERE CurDate = @FMStart --'11/03/2008'
--AND ACD.ItemNo = ItemBranchUsage.ItemNo
--AND ACD.Branch = ItemBranchUsage.Location
--AND ACD.ItemNo = IM.ItemNo

-- Set Ending On Hand
--UPDATE ItemBranchUsage SET
--[CurEndOHQty] = BegQOH, [CurEndOHDol] = (BegQoh * BegAC),
--[CurEndOHWght] = BegQOH * [Wght]
--FROM PFCAC.dbo.AvgCst_DailyHist ACD (NOLOCK) , ItemMaster  IM (NOLOCK)
--WHERE  CurDate = @FMEND --'11/03/2008'
--AND ACD.ItemNo = ItemBranchUsage.ItemNo
--AND ACD.Branch = ItemBranchUsage.Location
--AND ACD.ItemNo = IM.ItemNo
-- Set Non recorded usage
