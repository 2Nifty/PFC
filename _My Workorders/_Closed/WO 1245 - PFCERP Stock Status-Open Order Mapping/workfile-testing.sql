...
--select	AllocationDt, 
--	CarrierCd, CarrierName,
--	OrderFreightCd, OrderFreightName,
--	* from  POHeader --order by AllocationDt
--where CarrierCd<>'' and CarrierCd is not null
...
select * from  POComments

--------------------------------------------------------------------

select	OrderCarrier, OrderCarName,
	OrderFreightCd, OrderFreightName,
	--AllocDt,
	--HeaderStatus,
	*
from	SOHeader
--where AllocDt is not null or HeaderStatus is not null
--order by OrderCarrier
...
select	CarrierCd, FreightCd,
	--RqstdShipDt,
	--QtyOrdered,
	--QtyShipped,
	*
from	SODetail
--order by RqstdShipDt
...
select * from SOCommentsRel

select * from SOExpense
select * from SOExpenseRel
...
select * from ErrorLog order by ErrorDesc


select  * from SOAllocation 
where not exists (select * from ErrorLog where SOAllocation.OrderNo=ErrorLog.DocumentNo) 


--------------------------
EXEC pSOEProcessAllocationBatch
...
truncate table SODetailRel




ALTER TABLE [dbo].[SODetailRel] DROP CONSTRAINT [fkfSOHeaderRelID]
GO

ALTER TABLE [dbo].[SOCommentsRel] DROP CONSTRAINT [FKSOCommRelfSOHeadRelID]
GO




ALTER TABLE [dbo].[SODetailRel]  WITH NOCHECK ADD  CONSTRAINT [fkfSOHeaderRelID] FOREIGN KEY([fSOHeaderRelID])
REFERENCES [dbo].[SOHeaderRel] ([pSOHeaderRelID])
GO
ALTER TABLE [dbo].[SODetailRel] CHECK CONSTRAINT [fkfSOHeaderRelID]
GO

ALTER TABLE [dbo].[SOCommentsRel]  WITH NOCHECK ADD  CONSTRAINT [FKSOCommRelfSOHeadRelID] FOREIGN KEY([fSOHeaderRelID])
REFERENCES [dbo].[SOHeaderRel] ([pSOHeaderRelID])
GO
ALTER TABLE [dbo].[SOCommentsRel] CHECK CONSTRAINT [FKSOCommRelfSOHeadRelID]
GO
