...
DECLARE SOALLOC CURSOR FOR
	SELECT	fSOHeaderID, OrderSrcTable, EntryID, AllocDt,
		OrderCarrier, HeaderStatus, ErrorLogID,
		ChangeID, ChangeDt, StatusCd
	FROM	SOAllocation
	WHERE	AllocDt IS NULL and HeaderStatus IS NULL
	FOR UPDATE OF AllocDt, HeaderStatus, ErrorLogID, ChangeID, ChangeDt

OPEN SOALLOC
FETCH NEXT FROM SOALLOC INTO @orderID, @table, @username, @AllocDt, @carrierCd,
			     @HeaderStatus, @ErrorLogID, @ChangeID, @ChangeDt, @holdCd
...
WHILE (@@FETCH_STATUS = 0)
	...
	IF @table = 'SOHeader'
	    BEGIN
		UPDATE SODetail SET CarrierCd = @carrierCd WHERE fSOHeaderID = @orderID AND CarrierCd IS NULL 
		EXEC @result=pSOEAllocOrderBatch @orderID = @orderID, @userName = @username, @holdCd = @holdCd
		IF @Result <> 1 --Locked by another user
			UPDATE	SOAllocation
			SET	ErrorLogID = @result, ChangeID = 'pProcesAlloc', ChangeDt = GetDate(), AllocDt = @today,
				HeaderStatus= CASE WHEN @result = 0 THEN 'S' ELSE 'F' END
			WHERE	CURRENT of SOAlloc
	    END --Table SOHeader
	ELSE IF (@table = 'SOHeaderRel')
		BEGIN
		    DECLARE @subType Varchar(10)
		    SELECT @subType = SubType FROM SOHeaderRel WHERE pSOHeaderRelID = @orderID
		    --Re-extend the sales Order
		    EXEC @result = pSOEExt @orderID = @orderID, @line = 0, @type = 'SUM', @table = 'Rel'
		    IF (@subtype < '51')
			BEGIN
			    --Allocate the released order
			    EXEC @result = pSOEMaintainAllocations @orderID = @orderID, @function = 'ALLOC', @username  = @userName, @caller = 'STANDALONE'
			    IF @Result <> 1
				BEGIN --Locked by another user
				    UPDATE SOAllocation
				    SET    ErrorLogID = @result, ChangeID = 'pProcessAlloc', ChangeDt = GetDate(), AllocDt = @today,
					   HeaderStatus = CASE WHEN @result = 0 THEN 'S' ELSE 'F' END 
				    WHERE  CURRENT of SOAlloc
				    --Update CustomerMetrics
				    --EXEC pSOEProcessCustomerMetrics @orderID = @orderID, @lineNo = 0, @table = 'SOHeaderRel'
				...
				END --result <> 1
			END --Order SubType < '51'
		    ELSE IF (@subType > '50')
		        BEGIN
			    IF (@holdCd IS NOT NULL)
				BEGIN
				    UPDATE SOHeaderRel
				    SET    HoldReason = CASE WHEN @subType = '53' THEN 'RG'
				    ...
				    UPDATE SOHeaderRel
				    SET    HoldReasonName = ISNULL(ShortDsc,'N/A')
				    FROM   Tables
		    ...
		    ----ELSE
				...
				FROM	    SoheaderRel
				WHERE	    OrderNo = @orderID --Flag the RGA Order for Invoicing

			    --Set SOAllocation Processed for SubType > 50
			    UPDATE SOAllocation
			    SET    ErrorLogID = 0, ChangeID = 'pProcessAlloc', ChangeDt = GetDate(),
				   AllocDt = @today, HeaderStatus= 'S' where Current of SOAlloc
