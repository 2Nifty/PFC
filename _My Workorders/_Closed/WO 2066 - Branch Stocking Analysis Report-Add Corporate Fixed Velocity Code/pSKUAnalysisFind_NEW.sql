--exec pSKUAnalysisFind '00050', '00050', '20', '20', '1', '1', '01;10000:02;100000000:03;-727379968:'


drop proc pSKUAnalysisFind
go
...
BEGIN
	...
	CREATE TABLE #SKUGrid (
		...
		[CFV] varchar(50) NOT NULL default ('') ,
	...
	WHILE @@FETCH_STATUS = 0
		...
		declare SKUItems cursor  for
			...
			,[Corp Fixed Velocity Code] as CFV
			...
			order by SortKey
		OPEN SKUItems;
		FETCH NEXT FROM SKUItems into @Item, @SizeDesc, @CFV, @SVC;
		WHILE @@FETCH_STATUS = 0
			-- Do updates if required
			BEGIN
			if @ColCtr = 1 
				begin update #SKUGrid set [Col01] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col01]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 2 
				begin update #SKUGrid set [Col02] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col02]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 3 
				begin update #SKUGrid set [Col03] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col03]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 4 
				begin update #SKUGrid set [Col04] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col04]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 5
				begin update #SKUGrid set [Col05] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col05]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 6 
				begin update #SKUGrid set [Col06] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col06]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 7 
				begin update #SKUGrid set [Col07] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col07]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 8 
				begin update #SKUGrid set [Col08] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col08]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 9 
				begin update #SKUGrid set [Col09] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col09]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 10 
				begin update #SKUGrid set [Col10] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col10]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 11 
				begin update #SKUGrid set [Col11] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col11]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 12 
				begin update #SKUGrid set [Col12] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col12]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 13 
				begin update #SKUGrid set [Col13] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col13]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 14 
				begin update #SKUGrid set [Col14] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col14]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 15 
				begin update #SKUGrid set [Col15] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col15]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 16 
				begin update #SKUGrid set [Col16] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col16]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 17 
				begin update #SKUGrid set [Col17] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col17]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 18 
				begin update #SKUGrid set [Col18] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col18]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 19 
				begin update #SKUGrid set [Col19] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col19]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 20 
				begin update #SKUGrid set [Col20] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col20]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 21 
				begin update #SKUGrid set [Col21] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col21]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 22 
				begin update #SKUGrid set [Col22] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col22]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 23 
				begin update #SKUGrid set [Col23] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col23]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 24 
				begin update #SKUGrid set [Col24] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col24]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 25 
				begin update #SKUGrid set [Col25] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col25]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 26 
				begin update #SKUGrid set [Col26] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col26]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 27 
				begin update #SKUGrid set [Col27] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col27]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 28 
				begin update #SKUGrid set [Col28] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col28]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 29 
				begin update #SKUGrid set [Col29] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col29]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 30 
				begin update #SKUGrid set [Col30] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col30]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			if @ColCtr = 31 
				begin update #SKUGrid set [Col31] = @SVC where [Item] = @Item;
				if @@rowcount = 0 insert #SKUGrid ([Item], [SizeDesc], [CFV], [Col31]) values (@Item, @SizeDesc, @CFV, @SVC); end;
			FETCH NEXT FROM SKUItems into @Item, @SizeDesc, @CFV, @SVC;
			end;
		CLOSE SKUItems;
		DEALLOCATE SKUItems;
