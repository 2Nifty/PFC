...
--Detail
SELECT DISTINCT	ItemNo, LocationCode, CorpFixedVelCode, Cast(Use_30Day_Qty * Net_Wgt as DECIMAL(38,6)) as ExtSoldWght, CAST(Avail_Qty as Integer) as AvailQty,
		...
		Cast (0 as DECIMAL(38,6)) as CriticalWghtPct
...
SELECT		CorpFixedVelCode, COUNT(CorpFixedVelCode) as TotCount, SUM(CriticalFlag) as CriticalCount,
		...
		NonCriticalWghtPct = 
		  ...
		  END,
		CAST(0 as DECIMAL(18,1)) as TargetPct, LocationCode
...
select * from CriticalItemDetail where CorpFixedVelCode='I' and ExtSoldWght<0
...
--Detail
SELECT DISTINCT	ItemNo, LocationCode, CorpFixedVelCode, Cast(Use_30Day_Qty * Net_Wgt as DECIMAL(38,6)) as ExtSoldWght, Avail_Qty as AvailQty,
		Use_30Day_Qty as TotUse30, Avail_Qty - Use_30Day_Qty as CriticalQty, 0 as CriticalFlag,
		0 as NonCriticalFlag, Cast((Avail_Qty - Use_30Day_Qty) * Net_Wgt as DECIMAL(38,6)) as CriticalWght, Cast(Use_30Day_Qty * Net_Wgt as DECIMAL(38,6)) as NonCriticalWght
INTO		CriticalItemDetail
FROM		CPR_Daily
WHERE		CorpFixedVelCode<>'' --and ItemNo<>LocationCode --and Use_30Day_Qty >= 0
ORDER BY	CorpFixedVelCode, ItemNo
...
--select * from TEMPCriticalItemSummary --order by CorpFixedVelCode, LocationCode
select * from CriticalItemSummary


--select * from TEMPCriticalItemDetail
select * from CriticalItemDetail



--combine tables
INSERT TEMPCriticalItemDetail
Select * from CriticalItemDetail

INSERT TEMPCriticalItemSummary
Select * from CriticalItemsummary




--select * from TEMPCriticalItemDetail where CorpFixedVelCode='A'
...
FROM		CPR_Daily INNER JOIN
		CriticalItemDetail ON CPR_Daily.ItemNo = CriticalItemDetail.ItemNo
WHERE		CriticalItemDetail.CriticalFlag = '0' and CriticalItemDetail.LocationCode='SUMM'

UPDATE		CPR_Daily
SET		Critical = 'Y'
FROM		CPR_Daily INNER JOIN
		CriticalItemDetail ON CPR_Daily.ItemNo = CriticalItemDetail.ItemNo
WHERE		CriticalItemDetail.CriticalFlag = '1' and CriticalItemDetail.LocationCode='SUMM'
...
--select * from TEMPCriticalItemSummary --order by CorpFixedVelCode, LocationCode
select * from CriticalItemSummary order by CorpFixedVelCode


--select * from TEMPCriticalItemDetail
select * from CriticalItemDetail order by ItemNo, CorpFixedVelCode


select top 50 ItemNo, CorpFixedVelCode, LocationCode from CPR_Daily order by ItemNo, LocationCode
