...
CREATE PROCEDURE pSelectedSKUCorp
	@StrLoc varchar(4),
	@EndLoc varchar(4),
...
exec pSelectedSKUCorp '00','00','~','00909','00909','~','0000','9999','~','000','999','~','A','z','~','A','z','~','0','z','01/01/1970','12/31/2120','AFTER'
...
------------------------------------
-- Build #tItemBranch IB (Carson) --
------------------------------------
print 'Build #tItemBranch IB (Carson)'

	SELECT	IM.ItemNo,
			isnull(IB.StockInd,0) as StockInd,
			isnull(IB.UnitCost,0) as UnitCost,
			IB.SalesVelocityCd,
...
---------------------------------
-- Build #tItemBranchSum IBSum --
---------------------------------
print 'Build #tItemBranchSum IBSum'

	SELECT	IM.ItemNo,
			SUM(isnull(IB.Capacity,0)) as Capacity,
			SUM(isnull(IB.ReOrderPoint,0)) as ReOrderPoint
	INTO	#tItemBranchSum
	FROM	ItemMaster IM (NoLock) INNER JOIN
			ItemBranch IB (NoLock)
	ON		IM.pItemMasterID = IB.fItemMasterID
...
print 'Return results'
	...
	SELECT	DISTINCT
			...
			--Orig Cntr Qty/UM
--			isnull(UM.AltSellQty,0) as AltSellQty,
--			UM.AltSellUM,
--			cast(cast(isnull(UM.AltSellQty,0) as DECIMAL(12,2)) as VARCHAR(20)) + '/' + UM.AltSellUM as AltSell,

			--Updated Cntr Qty/UM
			isnull(UM.BaseStkQty,0) as BaseStkQty,
			UM.AltSellUM,
			cast(cast(isnull(UM.BaseStkQty,0) as DECIMAL(12,2)) as VARCHAR(20)) + '/' + UM.AltSellUM as BaseAlt,
			
			--SuperQty/UM
			isnull(UM.SuperQty,0) as SuperQty,
			UM.SuperUM,
			cast(cast(isnull(UM.SuperQty,0) as DECIMAL(12,2)) as VARCHAR(20)) + '/' + UM.SuperUM as SuperUMQty,
			...
			IM.Tariff,															--Harmonizing tax code
	...
	ON		IM.ItemNo = POQty.ItemNo LEFT OUTER JOIN
	...
	WHERE	----Location
			...
			--Web Enabled
			(isnull(WebEnabledInd,'0') >= @StrWeb AND isnull(WebEnabledInd,'0') <= @EndWeb) and
