--select BOLNO, RefSONo, SOHeaderRel.*
--from SOHeaderRel inner join
--	SODetailRel
--on	pSOHeaderRelID = fSOHeaderRelID 
--where OrderType='TO' and BOLNO <>'' and BOLNO is not null
--order by OrderNo
...
--Prime Vendor Code
SELECT	Item, isnull(CONVERT(decimal(18, 5), LEFT(Sub1, CHARINDEX(';', Sub1) - 1)),0) AS AntLandCost,
...
SELECT	TOHDR.OrderNo AS [TO #], TOLIN.LineNumber AS [Line No_],
	(SELECT	COUNT(LinCnt.LineNumber)
	 FROM	SOHeaderRel HdrCnt INNER JOIN
		SODetailRel LinCnt
	 ON	HdrCnt.pSOHeaderRelID = LinCnt.fSOHeaderRelID 
	 WHERE	HdrCnt.OrderType = 'TO' AND LEFT(LinCnt.ItemNo, 5) = LEFT(TOLIN.ItemNo, 5) AND HdrCnt.BOLNO = TOHDR.BOLNO --AND
--		LinCnt.[Unit of Measure Code] = TOLIN.[Unit of Measure Code]
) AS CatLineCount,

	CAT.No_ AS [Cat No], ITEM.SellStkUM, TOLIN.QtyOrdered AS Quantity,
...
--	TOLIN.[Qty_ Shipped (Base)] AS [Base Qty Shipped],
	...
	CAT.[Description] AS [Cat Desc], TOHDR.BOLNO AS [BL No],
	TOLIN.ItemNo AS [Item No_], TOHDR.OrderLoc AS [Transfer-from Code], TOLIN.GrossWght * TOLIN.QtyOrdered AS [Gross Wgt Ext],
	TOLIN.NetWght * TOLIN.QtyOrdered AS [Net Wgt Ext], TOLIN.NetWght * 0.4536 * TOLIN.QtyOrdered AS [Net Wgt Ext (KG)],
	SKU.UnitCost, SKU.UnitCost * TOLIN.QtyOrdered AS [Avg Cost Ext], tLandedCost.AntLandCost, tLandedCost.[Direct Unit Cost],
	(tLandedCost.AntLandCost - tLandedCost.[Direct Unit Cost]) / tLandedCost.[Direct Unit Cost] * 100 AS [Landed Adder %], DUTY.Rate AS [Duty %],
	SKU.UnitCost - SKU.UnitCost * ((tLandedCost.AntLandCost - tLandedCost.[Direct Unit Cost]) / tLandedCost.[Direct Unit Cost]) AS AvgCostAdders,
	...
	ITEM.Tariff, TOLIN.GrossWght AS [Gross Weight], TOLIN.NetWght AS [Net Weight]
...
ON	ITEM.Tariff = DUTY.Code RIGHT OUTER JOIN
	SOHeader TOHDR INNER JOIN
	SODetail TOLIN
ON	TOHDR.pSOHeaderID = TOLIN.fSOHeaderID INNER JOIN
...
select * from tLandedCostLines order by [Line No_]
exec sp_columns tLandedCostLines


UPDATE SOHeader set BOLNO='BAD' where OrderNo=1005476
