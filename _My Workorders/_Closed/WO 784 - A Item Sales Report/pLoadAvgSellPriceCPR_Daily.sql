...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[pLoadAvgSellPriceCPR_Daily]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[pLoadAvgSellPriceCPR_Daily]
GO
...
CREATE  PROCEDURE dbo.pLoadAvgSellPriceCPR_Daily
as

----pLoadAvgSellPriceCPR_Daily----
----------------------------------

--Written By: Tod Dixon - 2008-June
--Application: CPR
--Approx runtime: 30 minutes

--Three steps repeated once for CORP and once for all other locations
  --Use FiscalCalendar to define last 6 fiscal months
  --Use SOHeaderHist & SoDetailHist to create temp table [t6MthAvgSellPrice]
  --t6MthAvgSellPrice is updated to contain up to date values for 5 AvgSellPriceFields
--t6MthAvgSellPrice is then used to update the 5 AvgSellPriceFields is CPR_Daily
