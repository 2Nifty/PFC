...
--6 Months 
SELECT	InvoiceNo, SODetailHist.LineReasonDsc, OrderDt, SellToCustNo, OrderLoc, ShipLoc, UsageLoc,
	SODetailHist.ItemNo, SODetailHist.QtyOrdered, SODetailHist.NetUnitPrice,
	SODetailHist.QtyOrdered * SODetailHist.NetUnitPrice as NetLinePrice
FROM	SOHeaderHist INNER JOIN
	SODetailHist ON SOHeaderHist.pSOHeaderHistID = SODetailHist.fSOHeaderHistID
--Where	OrderDt >= '2007-11-25' and OrderDt <= '2008-05-31'
Where	OrderDt >= @BeginDate and OrderDt <= @EndDate

--Last Week of Line Items
SELECT	InvoiceNo, SODetailHist.LineReasonDsc, OrderDt, SellToCustNo, OrderLoc, ShipLoc, UsageLoc,
	SODetailHist.ItemNo, SODetailHist.QtyOrdered, SODetailHist.NetUnitPrice,
	SODetailHist.QtyOrdered * SODetailHist.NetUnitPrice as NetLinePrice
FROM	SOHeaderHist INNER JOIN
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].t6MoAvgSellPrice') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
	drop table t6MoAvgSellPrice