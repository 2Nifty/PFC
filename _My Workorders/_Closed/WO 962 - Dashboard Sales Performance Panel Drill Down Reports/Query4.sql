...
WHILE (@LoopCount <= @DateCount)
   BEGIN
	...
	--Daily records by Category & Location (DashboardCatLocDaily) - WEEKDAY
	INSERT	DashboardCatLocDaily(CategoryGroup, Location, ARPostDt, SalesDollars, Lbs, SalesPerLb, Cost, MarginDollars, MarginPct, MarginPerLb,
...
go
	...
	SELECT	isNULL(CategoryGroup,BudgetCat) AS CategoryGroup, isNULL(Location,BudgetLoc) AS Location,
		isNULL(ARPostDt,@CurBegDay) AS ARPostDate,
		isNULL(TotSales,0) AS SalesDollars, isNULL(TotWght,0) AS Lbs, isNULL(TotSalesPerLb,0) AS SalesPerLb,
		isNULL(TotCost,0) AS Cost, isNULL(TotMgn,0) AS MarginDollars, isNULL(TotMgnPct,0) AS MarginPct,
		isNULL(TotMgnPerLb,0) AS MarginPerLb, isNULL(TotMgn - (BudgetExp / @PerDays),0) AS Profit,
		isNULL(BudgetLbs / @PerDays,0) AS BudgetLbs,
...
SELECT	DISTINCT LEFT(ItemNo, 5) AS CategoryGroup, CustShipLoc AS Location, ARPostDt, InvoiceNo,
		SUM(NetUnitPrice * QtyShipped) AS TotSales,
		SUM(GrossWght * QtyShipped) AS TotWght,
		CASE SUM(GrossWght * QtyShipped)
		   WHEN 0 THEN 0
	...
	FROM	SOHeaderHist INNER JOIN
		SODetailHist ON SOHeaderHist.pSOHeaderHistID = SODetailHist.fSOHeaderHistID
	WHERE	ARPostDt = @CurEndDay
	GROUP BY LEFT(ItemNo, 5), CustShipLoc, ARPostDt, InvoiceNo


--SET @CurBegDay = '2008-10-16'
--SET @CurEndDay = '2008-10-19'
...
IF (@CurBegDay = @CurEndDay)
   BEGIN
	SET @CurPer = (SELECT FiscalPeriod FROM FiscalCalendar WHERE CurrentDt = @CurBegDay)
	SET @CurMthBeg = (SELECT CurFiscalMthBeginDt FROM FiscalCalendar WHERE CurrentDt = @CurBegDay)
	SET @CurMthEnd = (SELECT CurFiscalMthEndDt FROM FiscalCalendar WHERE CurrentDt = @CurBegDay)
	SET @PerDays = (SELECT SUM(WorkDay) FROM FiscalCalendar WHERE CurrentDt >= @CurMthBeg AND CurrentDt <= @CurMthEnd GROUP BY FiscalPeriod)
...
ELSE
   BEGIN
	...
	--Daily records by Category & Location (DashboardCatLocDaily) - WEEKEND
	SELECT	isNULL(CategoryGroup,BudgetCat) AS CategoryGroup, isNULL(Location,BudgetLoc) AS Location,
		isNULL(ARPostDt,@CurEndDay) AS ARPostDate,
		isNULL(TotSales,0) AS SalesDollars, isNULL(TotWght,0) AS Lbs, isNULL(TotSalesPerLb,0) AS SalesPerLb,
		isNULL(TotCost,0) AS Cost, isNULL(TotMgn,0) AS MarginDollars, isNULL(TotMgnPct,0) AS MarginPct,
		isNULL(TotMgnPerLb,0) AS MarginPerLb, isNULL(TotMgn - (BudgetExp / @PerDays),0) AS Profit,
		isNULL(BudgetLbs / @PerDays,0) AS BudgetLbs,
	...
	(SELECT	CategoryGroup AS BudgetCat, Location AS BudgetLoc, SUM(Lbs) AS BudgetLbs,
	 ...
	 GROUP BY CategoryGroup, Location) Budget
	ON	CategoryGroup = BudgetCat AND Location = BudgetLoc
	WHERE	isNULL(TotSales,0) <> 0 
--		or isNULL(TotWght,0) <> 0 or isNULL(TotSalesPerLb,0) <> 0 or isNULL(TotCost,0) <> 0 or isNULL(TotMgn,0) <> 0 or isNULL(TotMgnPct,0) <> 0 or isNULL(TotMgnPerLb,0) <> 0 or isNULL(TotMgn - (BudgetExp / @PerDays),0) <> 0
	ORDER BY CategoryGroup, Location
   END
...
DECLARE	@CurBegDay DATETIME,	--Current DashBoard Begin Date
	@CurEndDay DATETIME,	--Current DashBoard End Date
	@CurPer Int,		--Current Period Number
	@CurMthBeg DATETIME,	--Beginning Date for the Current Period
	@CurMthEnd DATETIME,	--Ending Date for the Current Period
	@PerDays Int		--Number of work days in the Current Period

SET @CurBegDay = (SELECT BegDate FROM DashBoardRanges WHERE DashBoardParameter = 'CurrentDay')
SET @CurEndDay = (SELECT EndDate FROM DashBoardRanges WHERE DashBoardParameter = 'CurrentDay')

--SET @CurBegDay = '2008-10-18'
