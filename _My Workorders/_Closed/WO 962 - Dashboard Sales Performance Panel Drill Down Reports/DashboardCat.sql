...
--Current Day
SELECT	CategoryGroup, TotSales, TotWght, TotSalesPerlb, TotCost,
	TotMgn, TotMgnPct, TotMgnPerLb, TotMgn - (BudgetExp / @@PerDays) AS Profit,
	...
	END AS BudgetMgnPct,
	isNULL(BudgetExp / @@PerDays,0) AS BudgetExp
FROM
(SELECT	DISTINCT LEFT(ItemNo, 5) AS CategoryGroup,
...
(SELECT	CategoryGroup AS BudgetCat, SUM(Lbs) AS BudgetLbs, SUM(SalesDollars) AS BudgetSales,
 ...
 WHERE Period = 200809			--WHERE FiscalPeriod = @@CurPer
