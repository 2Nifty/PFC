...
-------------------------------------------------------------------------------------
	...
	UPDATE	OpenDataSource('SQLOLEDB','Data Source=PFCERPDB;User ID=pfcnormal;Password=pfcnormal').PERP.dbo.AppPref
	SET	AppOptionValue = 'TO1221171', ChangeID=System_user, ChangeDt=GetDate()
	WHERE	ApplicationCd='SOE' and AppOptionType='LastAutoTrfNo'
...
select RefSONo, * from OpenDataSource('SQLOLEDB','Data Source=PFCERPDB;User ID=pfcnormal;Password=pfcnormal').PERP.dbo.SOHeader where OrderType='TO'
order by RefSONo


select	fSOHeaderID, OrderNo, RefSONo
from	OpenDataSource('SQLOLEDB','Data Source=PFCERPDB;User ID=pfcnormal;Password=pfcnormal').PERP.dbo.SOHeader
Where RefSONo > 'TO1220050'


select fSOHeaderID, OrderNo, RefSONo
from OpenDataSource('SQLOLEDB','Data Source=PFCERPDB;User ID=pfcnormal;Password=pfcnormal').PERP.dbo.SOHeaderRel
Where fSOHeaderID >= 1006387
...
--	0 as DiscPct,
	...
	0 as NonTaxExpAmt,
	0 as TaxAmt,
	RIGHT('0995'+RIGHT('00'+NVHDR.[Transfer-to Code],2),6) as SellToCustNo,
	...
	END as CustShipLoc,
	RIGHT('0995'+RIGHT('00'+NVHDR.[Transfer-to Code],2),6) as ShipToCd,
...
FROM	OpenDataSource('SQLOLEDB','Data Source=EnterpriseSQL;User ID=pfcnormal;Password=pfcnormal').PFCLive.dbo.[Porteous$Transfer Line] NVLINE INNER JOIN
	SOHeader
...
SELECT	SOHeader.pSOHeaderID as fSOHeaderID,
	...
	[Date] as EntryDt --,
	--[User ID] as EntryID
FROM	OpenDataSource('SQLOLEDB','Data Source=EnterpriseSQL;User ID=pfcnormal;Password=pfcnormal').PFCLive.dbo.[Porteous$Inventory Comment Line] COMMLINE INNER JOIN
...
SELECT	SOHeader.pSOHeaderID as fSOHeaderID,
	...
	[User ID] as EntryID,
	[Date] as EntryDt
FROM	OpenDataSource('SQLOLEDB','Data Source=EnterpriseSQL;User ID=pfcnormal;Password=pfcnormal').PFCLive.dbo.[Porteous$Transfer Line Comment Line] COMMLINE INNER JOIN
	SOHeader ON COMMLINE.[No_] = SOHeader.RefSONo
WHERE	EXISTS (SELECT	RefSONo
		FROM	OpenDataSource('SQLOLEDB','Data Source=EnterpriseSQL;User ID=pfcnormal;Password=pfcnormal').PFCLive.dbo.tWO1433_LoadAutoTrf Trf
		WHERE	Trf.RefSONo = SOHeader.RefSONo COLLATE Latin1_General_CS_AS)
...
FROM	OpenDataSource('SQLOLEDB','Data Source=EnterpriseSQL;User ID=pfcnormal;Password=pfcnormal').PFCLive.dbo.[Porteous$Salesperson_Purchaser]
