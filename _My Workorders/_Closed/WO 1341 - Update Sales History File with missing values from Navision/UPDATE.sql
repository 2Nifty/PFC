...
WHERE	tSOHeaderHist.InvoiceNo = NVHDR.[No_] COLLATE SQL_Latin1_General_CP1_CI_AS
...
UPDATE	OpenDataSource('SQLOLEDB','Data Source=PFCSQLP;User ID=pfcnormal;Password=pfcnormal').PFCReports.dbo.tSOHeaderHist
...
SET	LineType = 'S',
	...
	IMLoc = tSOHeaderHist.ShipLoc,
	OriginalQtyRequested = tSODetailHist.QtyOrdered,
----	OriginalQtyRequested = ROUND(tSODetailHist.QtyOrdered,0,1),
--	OriginalQtyRequested =	CASE ROUND(NVLINE.[Original Qty],0,1)
--				   WHEN 0 THEN ROUND(tSODetailHist.QtyOrdered,0,1)
--				   ELSE ROUND(NVLINE.[Original Qty],0,1)
--				END,
----	QtyOrdered = ROUND(tSODetailHist.QtyOrdered,0,1),
----	QtyShipped = ROUND(tSODetailHist.QtyShipped,0,1),
	QtyBO = ROUND(NVLINE.[Back Order Qty],0,1),
----	ExtendedGrossWght = ROUND(tSODetailHist.QtyShipped,0,1) * tSODetailHist.GrossWght,
----	ExtendedNetWght = ROUND(tSODetailHist.QtyShipped,0,1) * tSODetailHist.NetWght,
----	ExtendedPrice = ROUND(tSODetailHist.QtyShipped,0,1) * tSODetailHist.NetUnitPrice,
----	ExtendedCost = ROUND(tSODetailHist.QtyShipped,0,1) * tSODetailHist.UnitCost,
...
--	DiscPct1 = NVLINE.[Line Discount %],
	DiscUnitPrice =	CASE WHEN NVLINE.[Line Discount %] > 100 THEN tSODetailHist.ListUnitPrice - (tSODetailHist.ListUnitPrice * 100 * 0.01)
			     WHEN NVLINE.[Line Discount %] < -100 THEN tSODetailHist.ListUnitPrice - (tSODetailHist.ListUnitPrice * -100 * 0.01)
			     ELSE tSODetailHist.ListUnitPrice - (tSODetailHist.ListUnitPrice * ROUND(NVLINE.[Line Discount %],0,1) * 0.01)
			END,
--	DiscUnitPrice = tSODetailHist.ListUnitPrice - (tSODetailHist.ListUnitPrice * NVLINE.[Line Discount %] * 0.01),
	UnitCost2 = 0,
	UnitCost3 = 0,
	RepCost = tSODetailHist.UnitCost,
	OECost = 0,
	CarrierCd = tSOHeaderHist.OrderCarrier,
	FreightCd = tSOHeaderHist.OrderFreightCd,
	LineExpdCd = tSOHeaderHist.OrderExpdCd,
	PriceCd = tSOHeaderHist.PriceCd,
	LineReason = tSOHeaderHist.ReasonCd,
...
FROM	PFCLive.dbo.[Porteous$Sales Invoice Line] NVLINE INNER JOIN
	tSOHeaderHist
ON	NVLINE.[Document No_] = tSOHeaderHist.InvoiceNo COLLATE Latin1_General_CS_AS
WHERE	tSOHeaderHist.pSOHeaderHistID = tSODetailHist.fSOHeaderHistID AND NVLINE.[Line No_] = tSODetailHist.LineNumber AND
...
FROM	PFCLive.dbo.[Porteous$Sales Invoice Line] NVLINE INNER JOIN
	tSOHeaderHist
ON	NVLINE.[Document No_] = tSOHeaderHist.InvoiceNo COLLATE Latin1_General_CS_AS INNER JOIN
	tWO1341ItemBranch ItemBr
ON	NVLINE.[No_] = ItemBr.ItemNo COLLATE Latin1_General_CS_AS AND tSOHeaderHist.ShipLoc = ItemBr.Location
WHERE	tSOHeaderHist.pSOHeaderHistID = tSODetailHist.fSOHeaderHistID AND NVLINE.[Line No_] = tSODetailHist.LineNumber AND
	((NVLINE.Type = 2) OR (NVLINE.Type = 1 AND NVLINE.No_ = '3021')) AND NVLINE.No_ <> ''	--Do we need this WHERE?
...
--	DiscPct1 = NVLINE.[Line Discount %],
	DiscUnitPrice =	CASE WHEN NVLINE.[Line Discount %] > 100 THEN tSODetailHist.ListUnitPrice - (tSODetailHist.ListUnitPrice * 100 * 0.01)
			     WHEN NVLINE.[Line Discount %] < -100 THEN tSODetailHist.ListUnitPrice - (tSODetailHist.ListUnitPrice * -100 * 0.01)
			     ELSE tSODetailHist.ListUnitPrice - (tSODetailHist.ListUnitPrice * ROUND([Line Discount %],0,1) * 0.01)
			END,
--	DiscUnitPrice = tSODetailHist.ListUnitPrice - (tSODetailHist.ListUnitPrice * NVLINE.[Line Discount %] * 0.01),
	UnitCost2 = 0,
	UnitCost3 = 0,
...
UPDATE	tSOExpenseHist
SET	ExpenseDesc = NVLINE.[Description],
	ChangeDt = GETDATE(),
	ChangeID = 'WO1341'
FROM	PFCLive.dbo.[Porteous$Sales Invoice Line] NVLINE 
WHERE	NVLINE.[Document No_] = tSOExpenseHist.DocumentLoc COLLATE Latin1_General_CS_AS AND NVLINE.[Line No_] = tSOExpenseHist.LineNumber AND
...
FROM	tSODetailHist INNER JOIN
	tSOHeaderHist
ON	tSODetailHist.fSOHeaderHistID = tSOHeaderHist.pSOHeaderHistID
WHERE	(tSOHeaderHist.OrderType IN ('1', '2'))
...
WHERE	TableType = 'Reas' AND tSOHeaderHist.ReasonCd = Reas.TableCd
...
WHERE	TableType = 'Pri' AND tSOHeaderHist.OrderPriorityCd = Pri.TableCd
...
WHERE	TableType = 'Expd' AND tSOHeaderHist.OrderExpdCd = Expd.TableCd
...
FROM	OpenDataSource('SQLOLEDB','Data Source=PFCERPDB;User ID=pfcnormal;Password=pfcnormal').PERP.dbo.Tables Expd
WHERE	TableType = 'Expd' AND tSODetailHist.LineExpdCd = Expd.TableCd
...
WHERE	tSODetailHist.ItemNo = Item.ItemNo
...
WHERE	tSODetailHist.ItemNo = Item.ItemNo AND UM.UM = Item.SellStkUM
...
FROM	OpenDataSource('SQLOLEDB','Data Source=PFCERPDB;User ID=pfcnormal;Password=pfcnormal').PERP.dbo.ItemMaster Item INNER JOIN
	OpenDataSource('SQLOLEDB','Data Source=PFCERPDB;User ID=pfcnormal;Password=pfcnormal').PERP.dbo.ItemUM UM ON Item.pItemMasterID = UM.fItemMasterID
WHERE	tSODetailHist.ItemNo = Item.ItemNo AND UM.UM = Item.SuperUM
...
FROM	OpenDataSource('SQLOLEDB','Data Source=PFCERPDB;User ID=pfcnormal;Password=pfcnormal').PERP.dbo.ItemMaster Item INNER JOIN
	OpenDataSource('SQLOLEDB','Data Source=PFCERPDB;User ID=pfcnormal;Password=pfcnormal').PERP.dbo.ItemUM UM ON Item.pItemMasterID = UM.fItemMasterID
WHERE	tSODetailHist.ItemNo = Item.ItemNo AND UM.UM = Item.SellUM
...
FROM	(SELECT	RefSONo,
		SUM(tSODetailHist.QtyShipped * tSODetailHist.UnitCost2) as OrderExt2
	 FROM	tSODetailHist INNER JOIN
		tSOHeaderHist
	 ON	tSODetailHist.fSOHeaderHistID = tSOHeaderHist.pSOHeaderHistID
	 Group By RefSONo) ExtOrder
WHERE	ExtOrder.RefSONo = tSOHeaderHist.RefSONo
...
FROM	(SELECT	RefSONo, SUM(tSODetailHist.QtyShipped * tSODetailHist.NetUnitPrice) as OrderExt
	 FROM	tSODetailHist INNER JOIN
		tSOHeaderHist
	 ON	tSODetailHist.fSOHeaderHistID = tSOHeaderHist.pSOHeaderHistID
	 Group By RefSONo) ExtOrder
...
select UnitCost2, QtyShipped from tSODetailHist

select TotalCost2 from tSOHeaderHist
Where TotalCost2 is not null and TotalCost2<>0



select  OrderType, OrderFreightCd, OrderFreightName from tSOHeaderHist
...
select BillToCustName, ShipToName, * from tSOHeaderHist where left([BillToCustName],8)='Porteous' and left([ShipToName],8)='Porteous'


exec sp_columns tSOExpenseHist

select DocumentLoc, LineNumber, ExpenseDesc, * from tSOExpenseHist
...
select * from tSOHeaderHist where ChangeID <> 'WO1341'
select * from tSODetailHist where ChangeID <> 'WO1341'


select count(*) from tSOHeaderHist

select OrderNo, fSOHeaderID,* from tSOHeaderHist
...
select count(DISTINCT tSODetailHist.pSODetailHistID)
FROM	OpenDataSource('SQLOLEDB','Data Source=PFCSQLP;User ID=pfcnormal;Password=pfcnormal').PFCLive.dbo.[Porteous$Sales Invoice Line] NVLINE INNER JOIN
	tSOHeaderHist
ON	NVLINE.[Document No_] = tSOHeaderHist.InvoiceNo INNER JOIN
	tSODetailHist on pSOHeaderHistID=tSODetailHist.fSOHeaderHistID --inner join
...
--	OpenDataSource('SQLOLEDB','Data Source=PFCERPDB;User ID=pfcnormal;Password=pfcnormal').PERP.dbo.ItemBranch ItemBr
--ON	Item.pItemMasterID = ItemBr.fItemMasterID
WHERE	NVLINE.No_ <> '' AND NVLINE.Type = 2 --AND tSOHeaderHist.ShipLoc = ItemBr.Location



select * from tSODetailHist where ItemNo<>'' and LineType=2


select InvoiceNo, * from tSOHeaderHist
order by InvoiceNo
